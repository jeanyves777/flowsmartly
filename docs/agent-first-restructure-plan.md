...
- **WS4 depth — progress (2026-06-27):**
  - (1) **Agent ACTS on each surface** ✅ — the agent has full CRUD tools and is **surface-aware** (a `surfaceContext` channel tells it which workspace is open so it acts in-context instead of showing a generic menu — see [[agent-operates-account-full-crud]]). Done: Brand (`update_brand_identity`), Sell (`add/update/delete_product`, `fulfill_order`), Outreach (`add/update/delete_contact`), Publish (existing post tools), Web (`update_website` — publish/unpublish/rename/SEO). **Remaining: Web DEEP content editing** (rewrite a page's copy, add a section — schema-heavy, via the builder's section APIs).
  ...
  - (5) **Sweep the last legacy edges** ✅ — new-design surfaces had no legacy navigation; fixed the leaks in the embedded Settings (Upgrade/Buy-credits → `/home/billing`) and redirected agent-tool links/deepLinks/messages to `/home/*` (sell/web/outreach/create/brand/billing). Vestigial `route` strings remain in `workspaces.ts` but are never navigated (key/label only). **Remaining (need new surfaces): `/settings/marketing`, `/campaigns`, `/automations`, `/pitch-board`.**
