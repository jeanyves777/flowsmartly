...
export function PricingShelf({ onStartFree }: Pick<V5PublicFooterProps, 'onStartFree'>) {
  ...
  const type = useTypeScale();
  const styles = useFooterStyles();
  // Account creation is a route in this app now (`/register`), so it is the
  // honest default for every plan CTA unless the host page overrides it.
  const startFree = onStartFree ?? goToSignup;
