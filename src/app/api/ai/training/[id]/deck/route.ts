...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const slides = body.deck.slides.slice(0, 40).map((s) => {
    ...
    return {
      ...
      cohostVideoUrl: s.cohostVideoUrl ?? p.cohostVideoUrl,
      ...
      avatarFloat: s.avatarFloat ?? p.avatarFloat,
      avatarX: s.avatarX ?? p.avatarX,
      avatarY: s.avatarY ?? p.avatarY,
      avatarW: s.avatarW ?? p.avatarW,
      imageFit: s.imageFit ?? p.imageFit,
      imageZoom: s.imageZoom ?? p.imageZoom,
