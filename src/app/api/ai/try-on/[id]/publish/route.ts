...
import { publishTryOnTake, isTryOnChannel, type TryOnChannelId } from "@/lib/try-on/publish";

/** POST — publish one try-on take. Body: { takeId, channels, caption?, scheduleAt? }. */
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const channels = (Array.isArray(body.channels) ? body.channels : []).map((c: unknown) => String(c)).filter(isTryOnChannel) as TryOnChannelId[];
  ...
  const res = await publishTryOnTake({ userId: session.userId, projectId: id, takeId, channels, caption, scheduledAt });
