...
import { generateTryOnTakes } from "@/lib/try-on/engines";

/** POST — batch-generate N try-on takes. Body: { count }. */
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const res = await generateTryOnTakes(id, session.userId, count);
