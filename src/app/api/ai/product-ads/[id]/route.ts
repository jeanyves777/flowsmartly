...
import { getAdProject, saveAdProject, deleteAdProject } from "@/lib/product-ads/store";
import { syncAdProject } from "@/lib/product-ads/engines";
import { clampAdDuration } from "@/lib/product-ads/types";
...
export async function GET(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const project = await getAdProject(id, session.userId);
  ...
  const synced = project.takes.some((t) => t.status === "rendering" || t.status === "queued")
    ? await syncAdProject(project, session.userId)
    : project;
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (typeof body?.productImageUrl === "string" || body?.productImageUrl === null) project.productImageUrl = body.productImageUrl;
  if (typeof body?.mood === "string") project.mood = body.mood.slice(0, 60);
  if (body?.aspect === "9:16" || body?.aspect === "1:1" || body?.aspect === "16:9") project.aspect = body.aspect;
  if (typeof body?.durationSec === "number") project.durationSec = clampAdDuration(body.durationSec);
  ...
  await saveAdProject(id, session.userId, project);
...
export async function DELETE(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!session) return NextResponse.json({ success: false, error: { message: "Unauthorized" } }, { status: 401 });
  const { id } = await params;
  const ok = await deleteAdProject(id, session.userId);
