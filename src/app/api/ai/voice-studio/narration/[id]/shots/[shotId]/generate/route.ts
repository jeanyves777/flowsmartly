...
import { patchShot } from "@/lib/voice-studio/store";
import { renderShot } from "@/lib/voice-studio/engines";

/** POST — render ONE shot (its read + its picture). Fire-and-forget; the canvas polls. */
export async function POST(_request: NextRequest, { params }: { params: Promise<{ id: string; shotId: string }> }) {
  ...
  const project = await patchShot(id, session.userId, shotId, {
    status: "rendering", progress: 4, error: null, renderHeartbeatAt: Date.now(),
  });
