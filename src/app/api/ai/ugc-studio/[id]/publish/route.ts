...
import { publishTake, isUgcChannel, type UgcChannelId } from "@/lib/ugc-studio/publish";

/** POST — publish one take to connected channels. Body: { takeId, channels, caption?, scheduleAt? }. */
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const channels = (Array.isArray(body.channels) ? body.channels : []).map((c: unknown) => String(c)).filter(isUgcChannel) as UgcChannelId[];
  ...
  const res = await publishTake({ userId: session.userId, projectId: id, takeId, channels, caption, scheduledAt });
