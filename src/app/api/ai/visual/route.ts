...
function buildRawBrandPrompt(params: PipelineParams): string {
  ...
  return [
    ...
    "You are a senior art director. Design a COMPLETE, professionally art-directed branded marketing graphic (not a plain photo with a caption) organized into clear zones: eyebrow/tagline, bold hero title, subhead, concise body, optional badge, CTA, and a styled contact area.",
    "BRAND COLORS (respect them — do not invent a palette): build the ENTIRE palette from the brand's OWN colors listed in the brand context below. The brand's PRIMARY color is the DOMINANT hue (backgrounds and large shapes); the secondary and accent colors carry supporting shapes, badges, the CTA, and dividers. Do NOT substitute a generic or stock scheme, and do NOT pull colors from any reference/template — the piece must read unmistakably as THIS brand at a glance. Use only the brand colors plus neutral white/black/near-neutral tones for text contrast. (Apply the colors visually only — never render hex codes or color names as text.)",
