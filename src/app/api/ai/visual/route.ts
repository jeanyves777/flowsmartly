...
function buildRawBrandPrompt(params: PipelineParams): string {
  ...
  return [
    ...
    "- TYPOGRAPHY AS DESIGN: treat type as a graphic element — vary size and weight for clear hierarchy, and set headlines on shaped backers (brush strokes, ribbons, color blocks) where it elevates the piece. Each headline/word appears exactly once — never echo the same word a second time in a script/overlay font behind or over itself.",
