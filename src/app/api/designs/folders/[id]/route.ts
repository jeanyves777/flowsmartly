...
// PUT /api/designs/folders/[id] - Rename design folder
export async function PUT(
  ...
  try {
    ...
    const updated = await prisma.designFolder.update({
      where: { id },
      data: { name: name.trim() },
    });
    ...
    console.error("Rename design folder error:", error);
    return NextResponse.json({ success: false, error: { message: "Failed to rename folder" } }, { status: 500 });
  }
}

// DELETE /api/designs/folders/[id] - Delete design folder (move designs to root)
export async function DELETE(
  ...
  try {
    ...
    // Move all designs to root (set folderId to null)
    ...
    await prisma.designFolder.updateMany({
      where: { parentId: id },
      data: { parentId: folder.parentId },
    });

    // Delete the folder
    await prisma.designFolder.delete({ where: { id } });

    return NextResponse.json({ success: true });
  } catch (error) {
    console.error("Delete design folder error:", error);
