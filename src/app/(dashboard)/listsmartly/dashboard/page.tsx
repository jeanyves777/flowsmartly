...
export default function ListSmartlyDashboardPage() {
  ...
  async function runAutopilotAction(
    ...
    try {
      ...
      if (action === "run_extra") resultMessage = json.data?.result?.message || "Extra run started. 250 credits were charged and the agent is working now.";
      if (action === "run_next") resultMessage = "Agent started. The live status panel will refresh as it inspects the directory workflow.";
      if (action === "prepare_queue") resultMessage = json.data?.result?.message || "Listing plan refreshed.";
      const toastTitle =
      ...
      toast({
        title: "Error",
        description: error instanceof Error ? error.message : "Failed to run listing operation",
        variant: "destructive",
      });
    } finally {
      ...
      <div className="space-y-5">
        ...
        {autopilotLoading ? (
          ...
          <div className="space-y-3">
                ...
                return (
                  ...
                  >
                    ...
                    <div className="divide-y divide-border border-t border-border">
                      {group.items.slice(0, 30).map((operation) => (
                        <div key={operation.listingId} className="flex flex-col gap-3 p-4 md:flex-row md:items-center md:justify-between">
                          <div className="min-w-0">
                          ...
                          <div className="flex shrink-0 flex-wrap gap-2">
                            {operation.listingUrl && (
                              <Button size="sm" variant="outline" asChild>
                                <a href={operation.listingUrl} target="_blank" rel="noopener noreferrer">
                                  <ExternalLink className="h-3.5 w-3.5 mr-1.5" />
                                  View
                                </a>
                              </Button>
                            )}
                            {operation.registrationUrl && operation.bucket === "registration_link" && (
                              <Button size="sm" asChild>
                                <a href={operation.registrationUrl} target="_blank" rel="noopener noreferrer">
                                  <ExternalLink className="h-3.5 w-3.5 mr-1.5" />
                                  Open
                                </a>
                              </Button>
                            )}
                            {operation.bucket === "registration_link" && (
                              <Button
                                size="sm"
                                variant="outline"
                                onClick={() =>
                      ...
                      {group.count > 30 && (
                        <div className="px-4 py-3 text-xs text-muted-foreground">
                          Showing 30 of {group.count}. Use Listings to search the full catalog.
                        </div>
                      )}
                    </div>
                  </details>
            ...
            {operationGroups.every((group) => group.count === 0) && (
              <Card>
