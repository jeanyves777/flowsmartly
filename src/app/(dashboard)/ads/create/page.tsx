...
function ThreadsIcon({ className, style }: { className?: string; style?: React.CSSProperties }) {
  return (
    <svg className={className} style={style} viewBox="0 0 24 24" fill="currentColor">
      <path d="M12.186 24h-.007c-3.581-.024-6.334-1.205-8.184-3.509C2.35 18.44 1.5 15.586 1.472 12.01v-.017c.03-3.579.879-6.43 2.525-8.482C5.845 1.205 8.6.024 12.18 0h.014c2.746.02 5.043.725 6.826 2.098 1.677 1.29 2.858 3.13 3.509 5.467l-2.04.569c-1.104-3.96-3.898-5.984-8.304-6.015-2.91.022-5.11.936-6.54 2.717C4.307 6.504 3.616 8.914 3.59 12c.025 3.086.718 5.496 2.057 7.164 1.432 1.783 3.631 2.698 6.54 2.717 2.623-.02 4.358-.631 5.8-2.045 1.647-1.613 1.618-3.593 1.09-4.798-.31-.71-.873-1.3-1.634-1.75-.192 1.352-.622 2.446-1.284 3.272-.886 1.102-2.14 1.704-3.73 1.79-1.202.065-2.361-.218-3.259-.801-1.063-.689-1.685-1.74-1.752-2.96-.065-1.17.408-2.243 1.33-3.023.88-.744 2.084-1.168 3.59-1.264 1.07-.068 2.065.018 2.94.221-.08-.56-.244-1.04-.494-1.427-.454-.704-1.233-1.074-2.315-1.099-.876-.02-1.726.237-2.2.515l-.95-1.67c.756-.438 1.935-.79 3.2-.763 1.578.037 2.822.572 3.597 1.549.637.804.993 1.864 1.085 3.229.457.14.882.316 1.27.53 1.168.647 2.06 1.606 2.578 2.772.86 1.94.91 4.627-1.165 6.66-1.762 1.726-4.028 2.477-7.143 2.5zm.36-9.088c-1.07.067-1.896.334-2.457.793-.501.411-.73.93-.697 1.544.039.702.462 1.268 1.19 1.597.637.287 1.433.381 2.19.343 1.093-.06 1.93-.459 2.488-1.187.415-.542.674-1.267.796-2.201-.928-.276-2-.398-3.15-.33l-.36-.559z" />
    </svg>
  );
}
...
const PLATFORM_META: Record<string, { name: string; icon: typeof FeedIcon; color: string; bgColor: string }> = {
  ...
  youtube: { name: "YouTube", icon: YouTubeIcon, color: "#FF0000", bgColor: "rgba(255,0,0,0.1)" },
  pinterest: { name: "Pinterest", icon: PinterestIcon, color: "#E60023", bgColor: "rgba(230,0,35,0.1)" },
  threads: { name: "Threads", icon: ThreadsIcon, color: "#000000", bgColor: "rgba(0,0,0,0.06)" },
};
...
export default function CreateCampaignPage() {
  ...
  const PLATFORMS = useMemo(() => {
    const platformOrder = ["feed", "instagram", "twitter", "linkedin", "facebook", "tiktok", "youtube", "pinterest", "threads"];
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <MediaUploader
                    value={mediaPreviewUrl ? [mediaPreviewUrl] : []}
                    onChange={(urls) => setMediaPreviewUrl(urls[0] || "")}
