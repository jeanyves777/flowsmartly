...
export default function NewProductPage() {
  ...
  // ── Upload images to S3 via product endpoint ──
  ...
  const uploadProductImages = async (productId: string): Promise<void> => {
    ...
    try {
      ...
      const res = await fetch(`/api/ecommerce/products/${productId}/images`, {
        method: "POST",
        body: formData,
      });
      const json = await res.json();
  ...
  const handleRemoveBackground = async (index: number) => {
    ...
    // Need either a URL (already uploaded) or a File to send
    if (!img.url && !img.file) {
    ...
    try {
      ...
      if (img.file) {
        ...
        // Fetch the image from the URL and send as file
      ...
      // Replace the image URL with the enhanced version
      setImages((prev) =>
        prev.map((item, i) =>
          i === index
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!json.success) {
        toast({ title: json.error?.message || "Failed to create product", variant: "destructive" });
        return;
      }
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-end gap-3 pb-8">
        ...
        >
          ...
          {uploadingImages ? "Uploading Images..." : saving ? "Saving..." : "Save Product"}
        </button>
      </div>
    </div>
  );
