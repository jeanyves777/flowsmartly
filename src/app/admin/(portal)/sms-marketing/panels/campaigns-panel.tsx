...
export function CampaignsPanel() {
  ...
  const fetchSmsCampaigns = useCallback(async () => {
    try {
      ...
      if (data.success) {
        // The route returns provider-neutral keys (sentCount, user, createdAt,
        // total, totalSmsSent) — normalize to the shape this panel renders.
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const rows: any[] = data.data?.campaigns || [];
        setCampaigns(
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          rows.map((c: any) => ({
            id: c.id,
            ...
            audience: c.recipientCount ?? c.audience ?? 0,
            sent: c.sentCount ?? c.sent ?? 0,
            opened: c.opened ?? 0,
            clicked: c.clickCount ?? c.clicked ?? 0,
            bounced: c.bounceCount ?? c.bounced ?? 0,
            startDate: c.createdAt ? new Date(c.createdAt).toLocaleDateString() : (c.startDate || ""),
            endDate: c.sentAt ?? c.endDate ?? null,
            owner: c.user?.name || c.user?.email || c.owner || "—",
            ownerId: c.user?.id || c.ownerId || "",
        ...
        setStats({
          totalCampaigns: s.total ?? s.totalCampaigns ?? 0,
          totalSent: s.totalSmsSent ?? s.totalSent ?? 0,
          totalDelivered: s.totalDelivered ?? 0,
          totalClicked: s.totalClicked ?? 0,
          avgDeliveryRate: s.avgDeliveryRate ?? 0,
  ...
  return (
    <div className="space-y-6">
      {/* Panel actions (the console owns the page title) */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <p className="text-sm text-muted-foreground">Every user&apos;s SMS campaigns — delivery &amp; click performance.</p>
