...
- DO NOT introduce new on-camera characters who weren't in the listed roster. Crowds + passersby stay generic, blurred, out-of-focus.`;
  ...
  return [
    ...
    "- PROP ORIENTATION (REAL-LIFE PRIVACY — CRITICAL): phones, laptops, tablets, books, papers, and any screen FACE THE PERSON USING THEM, exactly like real life where ONLY that person can see their own screen. The CAMERA must see the BACK of the phone/laptop (the lid, the rear shell) or the device edge-on — NEVER the lit screen content. A person scrolling holds the phone tilted toward their own eyes with the back of the phone toward us; a laptop user sits BEHIND the open lid so we see the lid/back, not the display; a reader's open pages face themselves. NEVER rotate a device so its screen faces the lens or the audience — the only exception is if the scene action explicitly says the character turns the screen to show another person.",
