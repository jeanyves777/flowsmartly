...
export async function draftFilmPipeline(filmId: string, userId: string): Promise<FilmProject | null> {
  ...
  const approx = Math.max(2, Math.min(8, Math.round(target / 8)));
  ...
  try {
    const json = await ai.generateJSON<{ scenes: { engine: string; title: string; script: string; durationSec: number }[] }>(
      ...
      `Plan ~${approx} scenes that build one cohesive story. For EACH scene pick the best engine:\n` +
      ...
      `Return JSON: {"scenes":[{"engine":"ai|avatar|design","title":"2-4 words","script":"...","durationSec":8}, ...]}.`,
  ...
  // Default avatar/voice so drafted avatar scenes are one-click generatable.
  const needsAvatar = planned.some((s) => s.engine === "avatar");
  let defAvatar: { id: string; name: string } | null = null;
  let defVoice: { id: string; name: string } | null = null;
  if (needsAvatar) {
    try {
      const [avatars, voices] = await Promise.all([listAvatarsForUser(), listVoicesForUser()]);
      if (avatars[0]) defAvatar = { id: avatars[0].id, name: avatars[0].name };
      if (voices[0]) defVoice = { id: voices[0].id, name: voices[0].name };
