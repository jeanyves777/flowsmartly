...
function emailScoreLabel(score: number) {
...
// Render a table-based progress bar (email-client safe)
function emailProgressBar(score: number, width = 280): string {
  const color = emailScoreColor(score);
  const fillPx = Math.round((score / 100) * width);
  const trackPx = width - fillPx;
  return `
    <table cellpadding="0" cellspacing="0" border="0" width="${width}" style="border-radius:4px;overflow:hidden;height:7px;">
      <tr>
        <td width="${fillPx}" style="background:${color};height:7px;border-radius:3px 0 0 3px;"></td>
        ${trackPx > 0 ? `<td style="background:#e5e7eb;height:7px;border-radius:0 3px 3px 0;width:${trackPx}px;"></td>` : ""}
      </tr>
    </table>`;
}
...
export async function sendPitchEmail(params: {
  ...
  const { to, recipientName, businessName, pitch, research, pdfBuffer, senderName, replyTo, customMessage } = params;
  const greeting = recipientName ? `Hi ${recipientName},` : `Hi there,`;
  const appUrl = process.env.NEXT_PUBLIC_APP_URL || "https://flowsmartly.com";

  // Compute score if research available
  ...
  // Score snapshot section (shown only if research data available)
      ...
      <tr>
        <td style="padding:16px 18px;">
          ...
          <table cellpadding="0" cellspacing="0" border="0" width="100%" style="margin-bottom:12px;">
            <tr>
              ...
              <td valign="middle" style="padding-left:12px;">
                <p style="margin:0 0 2px;font-size:14px;font-weight:700;color:${scoreColor};">${emailScoreLabel(overall)} Digital Presence</p>
                <p style="margin:0;font-size:11px;color:#6b7280;">Industry avg: <strong>52</strong> &nbsp;·&nbsp; Top performers: <strong>85</strong></p>
                <!-- Main bar -->
                <div style="margin-top:8px;">${emailProgressBar(overall, 260)}</div>
              </td>
            </tr>
          </table>
          ...
          <table cellpadding="0" cellspacing="6" border="0" width="100%">
            ${scoreData.categories.map((cat, i) => i % 2 === 0 ? `
            <tr>
              <td width="50%" style="padding-right:6px;">
                <table cellpadding="0" cellspacing="0" border="0" width="100%">
                  <tr>
                    <td style="font-size:10px;color:#374151;">${cat.name}</td>
                    <td style="font-size:10px;color:${emailScoreColor(cat.score)};font-weight:700;text-align:right;">${cat.score}</td>
                  </tr>
                  <tr><td colspan="2" style="padding-top:3px;">${emailProgressBar(cat.score, 130)}</td></tr>
                </table>
              </td>
              ${scoreData.categories[i + 1] ? `
              <td width="50%" style="padding-left:6px;">
                <table cellpadding="0" cellspacing="0" border="0" width="100%">
                  <tr>
                    <td style="font-size:10px;color:#374151;">${scoreData.categories[i + 1].name}</td>
                    <td style="font-size:10px;color:${emailScoreColor(scoreData.categories[i + 1].score)};font-weight:700;text-align:right;">${scoreData.categories[i + 1].score}</td>
                  </tr>
                  <tr><td colspan="2" style="padding-top:3px;">${emailProgressBar(scoreData.categories[i + 1].score, 130)}</td></tr>
                </table>
              </td>` : "<td></td>"}
            </tr>` : "").join("")}
          </table>
          ...
          ${gp?.rating !== undefined ? `
          <!-- Google rating mini bar -->
          <table cellpadding="0" cellspacing="0" border="0" width="100%" style="margin-top:12px;padding-top:10px;border-top:1px solid #e0eaff;">
            <tr>
              <td style="font-size:10px;color:#92400e;">⭐ Google Rating</td>
              <td style="font-size:12px;font-weight:700;color:${gp.rating >= 4.5 ? "#22c55e" : gp.rating >= 4.0 ? "#f59e0b" : "#ef4444"};text-align:right;">${gp.rating}/5.0 &nbsp;<span style="font-weight:400;color:#9ca3af;font-size:10px;">(${gp.reviewCount ?? 0} reviews)</span></td>
            </tr>
            <tr><td colspan="2" style="padding-top:4px;">${emailProgressBar(Math.round((gp.rating / 5) * 100), 280)}</td></tr>
          </table>` : ""}
        </td>
      </tr>
    </table>
  ...
  // Findings — numbered, short
  const findingsHtml = pitch.keyFindings.map((f, i) => `
    <table cellpadding="0" cellspacing="0" border="0" width="100%" style="margin-bottom:8px;">
      <tr valign="top">
        <td width="26" style="padding-top:1px;">
          <div style="width:20px;height:20px;border-radius:50%;background:#2563eb;text-align:center;line-height:20px;font-size:11px;font-weight:700;color:white;">${i + 1}</div>
        </td>
        <td style="font-size:13px;color:#374151;line-height:1.5;padding-left:4px;">${f}</td>
      </tr>
    </table>`).join("");

  const hiddenNote = (pitch.hiddenFindingsCount || 0) > 0
    ? `<p style="color:#9ca3af;font-size:12px;font-style:italic;margin:6px 0 0;">+ ${pitch.hiddenFindingsCount} more insights in the attached PDF.</p>`
    : "";
