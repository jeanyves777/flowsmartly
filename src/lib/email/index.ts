...
interface PitchEmailContent {
  ...
  brandPrimaryColor?: string;  // user's brand primary color for email accents
  brandWebsite?: string;       // user's brand website for CTA link
}
...
export function buildPitchEmailHtml(params: PitchEmailContent): string {
  const { recipientName, businessName, pitch, research, pdfBuffer, senderName, customMessage, brandPrimaryColor, brandWebsite } = params;
...
/** Helper to build the score section HTML (extracted to avoid duplication) */
