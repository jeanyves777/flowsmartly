/**
 * Email Service for FlowSmartly — sender functions.
 *
 * Infrastructure (SMTP config, transporter, baseTemplate, sendEmail) lives in
 * ./config and ./core. The pitch-board subsystem lives in ./pitch. This file
 * holds the per-category `send*Email` helpers and re-exports the public API
 * so callers importing from `@/lib/email` keep working.
 */

import { APP_NAME, APP_URL, EMAIL_CONFIG, FROM_EMAIL } from "./config";
import { baseTemplate, sendEmail } from "./core";

export { baseTemplate, sendEmail } from "./core";
export type { SendEmailParams } from "./core";
export { buildPitchEmailHtml, sendPitchEmail } from "./pitch";
...
export async function sendEngagementAlertEmail(params: {
  ...
  const content = `
    ...
    <p style="text-align: center;">
      <a href="${APP_URL}/post/${encodeURIComponent(params.postId)}" class="button">View Post</a>
    </p>
  `;
