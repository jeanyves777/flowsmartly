...
- Recently Viewed rail (MANDATORY): import { trackProductView } from "@/components/RecentlyViewed" and call trackProductView(product.id) in a useEffect on product mount. Render <RecentlyViewed excludeProductId={product.id} title="Recently Viewed" /> after the related-products section. Also render <RecentlyViewed title="Continue Browsing" /> on the home page, below FeaturedProducts. The component reads the store's recently-viewed IDs from localStorage and renders a grid of ProductCards — it renders nothing when the list is empty, so it's safe to include unconditionally.
