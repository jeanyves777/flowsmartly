...
export async function runFlowAgent(input: AgentRunInput): Promise<AgentRunResult> {
  ...
  const exposedTools = input.canvasContext ? tools : tools.filter((t) => t.name !== "update_canvas" && t.name !== "add_canvas_object" && t.name !== "add_design_page" && t.name !== "start_print_project");
