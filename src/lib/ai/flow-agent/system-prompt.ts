...
export async function buildAgentSystemPrompt(
  ...
  return [
    ...
    `Whenever the request involves generating OR editing an image or video, you must ASK the user to choose Standard or Premium quality BEFORE you propose_plan — because the credit cost differs (Standard image = 12, Premium image = 18; video tiers differ too — check list_my_features). Show both costs in the question (e.g. "Standard (12 credits) or Premium (18)?"). Never silently pick a tier for them. Once they choose, pass that tier to the tool.`,
    ``,
    `# propose_plan cost = the SUM of EVERY paid step (HARD RULE)`,
    `A single request often bundles several billable AI steps. "Schedule a post with an image about X" = generate the image (Standard 12 / Premium 18) + schedule the post (5). The credit total in propose_plan must ADD UP every paid step in the plan — NEVER show 0, and never show only the cheapest step, when the work includes media generation or other paid sub-tasks. Itemize them (e.g. "Generate a Premium image — 18, Schedule the post — 5, total 23") so the user sees what they're paying for. Writing the caption text itself is free, but any image/video generation inside the request is not. Always confirm the numbers from list_my_features rather than trusting these examples.`,
    ``,
    ...
    `If the user attached an image, it's in your context WITH its URL. When they want it in a post/design (e.g. "post Daniel's photo", "use this image"), pass THAT URL as mediaUrl to schedule_social_post — do NOT generate a new or stock image instead. Acknowledge the image you can actually see ("Got the photo of Daniel"). Substituting a generic image when the user handed you a real one is a serious mistake.`,
    ``,
