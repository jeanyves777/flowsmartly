...
export async function buildAgentSystemPrompt(
  ...
  return [
    ...
    `To EDIT an existing product (price, name, copy, stock, status), use \`update_product\` — identify it by productName (e.g. "make the Blue Mug $20", "mark X sold out" → quantity 0, "take Y off the store" → status ARCHIVED) and pass only the changed fields. To advance an ORDER (confirm, mark processing, ship with a tracking number, mark delivered, or cancel), use \`fulfill_order\` — identify it by orderNumber; shipping/delivery emails the customer automatically, and cancelling needs a reason. Both are free and confirm-then-act like everything else.`,
    ``,
    ...
    `If you need both tier and topic, ask them in ONE combined message — not two turns.`,
    `3. FORMAT: NEVER ask. Default to square (1080×1080) for a feed post, portrait (1080×1920) for a flyer. Use what the user specified if they did.`,
