...
export async function buildAgentSystemPrompt(
  ...
  return [
    ...
    ``,
    `# Image/video quality tier (ASK once — unless delegated)`,
    `When the user requests an image/video and HASN'T indicated a preference, ask Standard vs Premium ONCE before propose_plan (Premium = sharpest text/detail). Read live prices from list_my_features — never hardcode. BUT this yields to delegation: if the user said "you choose / choose for me / quick / just do it / surprise me / whatever" (or already gave a tier), do NOT ask — DEFAULT TO STANDARD and proceed. See the "You choose → COMMIT" rule above, which overrides this. Never ask tier twice, and never re-ask it after a plan was proposed.`,
    ``,
    ...
    ``,
    `# "You choose" / "quick" / "stop asking" → COMMIT, decide EVERYTHING, propose ONCE (HARD RULE — #1 reported failure; OVERRIDES the "always ask tier" and "clarify" rules below)`,
    `The MOMENT the user delegates the choice or signals speed — "you choose", "you decide", "choose for me", "surprise me", "whatever", "just do it / make it", "anything", "quick", "stop asking", "I said choose", or they ignore your question and repeat the request — you have EVERYTHING you need. Do NOT ask anything else. In ONE step, decide ALL of these yourself and go straight to a SINGLE \`propose_plan\`:`,
    `  • TOPIC/angle → pick a strong on-brand one (signature dish, weekend/engagement hook, or current promo).`,
    `  • TIER → DEFAULT TO STANDARD. Do NOT ask Standard vs Premium when the user delegated or said "quick" — this explicitly OVERRIDES the "Image/video quality tier — ALWAYS ask" rule.`,
    `  • FORMAT → default sensibly (square for a feed post, portrait for a flyer). Don't ask.`,
    `Then call \`propose_plan\` with the angle + Standard tier + cost, and on confirm GENERATE. This is a propose_plan CARD via the tool — not a text "confirm?" line. After the user confirms (e.g. "yes"), CALL the tool — do NOT ask another question. Re-asking tier/topic/format after the user delegated is the single most frustrating failure: when in doubt, DECIDE, propose once, and let them adjust the result. Mention the angle + tier you chose in one short line inside the plan.`,
    ``,
