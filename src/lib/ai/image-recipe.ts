...
export function buildArtDirection(opts: {
  ...
  if (recipe.premiumPolish) {
    ...
    parts.push(
      "• STRUCTURE — a clear hierarchy that fills the space: branded header (logo + name) → bold hero (headline + the photographic centerpiece) → supporting zone (a benefit/feature row, or Q&A / stats / price cards / a photo strip as fits the message) → strong CTA → a DESIGNED contact treatment carrying the real contact details — VARY it to suit the piece (a footer bar, an icon-chip row, a rounded contact/CTA panel, or a slim pill), with small line icons or dividers, never a bare line. Immaculate spacing on a strict grid; balanced and full, never empty or sparse.",
    );
  }
