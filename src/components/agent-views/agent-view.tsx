    ...
    <div className={cn("overflow-hidden rounded-2xl border border-border bg-card shadow-sm", className)}>
      {(spec.title || spec.badge || spec.icon) && (
        <div className="flex items-center gap-2.5 border-b border-border px-3.5 py-2.5">
          {spec.icon && <span className="grid h-7 w-7 shrink-0 place-items-center rounded-lg bg-gradient-to-br from-brand-500/20 to-violet-500/20 text-[15px]">{spec.icon}</span>}
          <div className="min-w-0">
            {spec.title && <p className="truncate text-[13px] font-bold leading-tight">{spec.title}</p>}
            {spec.subtitle && <p className="truncate text-[11px] text-muted-foreground">{spec.subtitle}</p>}
          </div>
...
function Block({ block, emit }: { block: ViewBlock; emit: Emit }): React.ReactElement | null {
  switch (block.type) {
    ...
    case "card": return <div className="rounded-xl border border-border bg-background/40 p-2.5">{block.children.map((c, i) => <Block key={i} block={c} emit={emit} />)}</div>;
    ...
    case "kpis": return <div className="grid gap-2" style={{ gridTemplateColumns: `repeat(${Math.min(block.items.length, 4)}, minmax(0,1fr))` }}>{block.items.map((k, i) => <div key={i} className="rounded-xl border border-border bg-background/40 p-2.5"><p className="text-[10px] text-muted-foreground">{k.label}</p><p className={cn("text-[16px] font-bold tabular-nums", t(k.tone))}>{k.value}</p></div>)}</div>;
    ...
    case "checklist": return <div className="space-y-1.5">{block.items.map((it, i) => <div key={i} className={cn("flex items-center gap-2 text-[12px]", it.state === "pending" ? "text-muted-foreground" : "text-foreground")}><span className={cn("grid h-4 w-4 shrink-0 place-items-center rounded-full border text-[9px]", it.state === "done" ? "border-emerald-500 bg-emerald-500/15 text-emerald-500" : it.state === "active" ? "border-brand-500 text-brand-500" : "border-border")}>{it.state === "done" ? "✓" : it.state === "active" ? "◜" : ""}</span><span>{it.text}</span>{it.sub && <span className="ms-auto font-mono text-[10px] text-muted-foreground">{it.sub}</span>}</div>)}</div>;
    ...
    case "table": return (
      <div className="overflow-x-auto"><table className="w-full text-[11.5px]"><thead><tr>{block.columns.map((c) => <th key={c.key} className={cn("pb-1.5 text-[10px] font-bold uppercase tracking-wide text-muted-foreground", c.align === "right" ? "text-right" : c.align === "center" ? "text-center" : "text-left")}>{c.label}</th>)}{(block.rowActions?.length || block.rowAction) && <th />}</tr></thead>
      <tbody>{block.rows.map((r, ri) => <tr key={ri}>{block.columns.map((c) => { const v = r[c.key]; return <td key={c.key} className={cn("border-t border-border py-1.5 pe-2 align-middle", c.align === "right" && "text-right", c.align === "center" && "text-center", c.kind === "score" && "font-mono font-bold text-emerald-500")}>{c.kind === "thumb" && typeof v === "string" ? <span className="inline-block h-6 w-6 overflow-hidden rounded"><Image src={v} alt="" width={24} height={24} className="h-full w-full object-cover" unoptimized /></span> : c.kind === "badge" ? <span className={cn("rounded px-1.5 py-0.5 text-[9.5px] font-bold", toneChipBg.brand)}>{String(v ?? "")}</span> : String(v ?? "")}</td>; })}{(block.rowActions?.length || block.rowAction) && <td className="border-t border-border py-1.5 text-right align-middle"><div className="flex justify-end gap-1">{(block.rowActions?.length ? block.rowActions : block.rowAction ? [{ label: block.rowActionLabel || "Select", action: block.rowAction }] : []).map((a, ai) => <button key={ai} onClick={() => emit({ action: a.action, value: r })} className={cn("whitespace-nowrap rounded-md border px-2 py-0.5 text-[10px] font-bold", ("variant" in a && a.variant === "primary") ? "border-transparent bg-brand-500 text-white" : "border-border hover:border-brand-500/60 hover:text-brand-500")}>{a.label}</button>)}</div></td>}</tr>)}</tbody></table></div>
    );
    ...
    case "buttonRow": return <div className="flex flex-wrap gap-2">{block.buttons.map((b, i) => <ActionButton key={i} label={b.label} variant={b.variant} icon={b.icon} onClick={() => emit({ action: b.action })} />)}</div>;
...
function ChipsBlock({ block, emit }: { block: Extract<ViewBlock, { type: "chips" }>; emit: Emit }) {
  ...
  return <div className="flex flex-wrap gap-1.5">{block.options.map((o) => <button key={o.value} onClick={() => pick(o.value)} title={o.hint} className={cn("rounded-full border px-2.5 py-1 text-[11px] font-semibold transition", sel.includes(o.value) ? "border-brand-500 bg-brand-500/10 text-brand-500" : "border-border text-muted-foreground hover:border-brand-500/40")}>{o.label}</button>)}</div>;
}
...
function InputBlock({ block, emit }: { block: Extract<ViewBlock, { type: "input" }>; emit: Emit }) {
  ...
  return (
    <div className="flex w-full items-end gap-1.5">
      ...
      {block.action && <button onClick={submit} className="grid h-9 w-9 shrink-0 place-items-center rounded-[10px] bg-gradient-to-r from-brand-500 to-violet-500 text-white">{block.submitLabel ? <span className="px-2 text-[12px] font-semibold">{block.submitLabel}</span> : "↑"}</button>}
    </div>
  );
}
...
function SlotGridBlock({ block, emit }: { block: Extract<ViewBlock, { type: "slotGrid" }>; emit: Emit }) {
  ...
  return (
    <div className="grid gap-1.5" style={{ gridTemplateColumns: `repeat(${block.days.length}, minmax(0,1fr))` }}>
      {block.days.map((d, di) => (
