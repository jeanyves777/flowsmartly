...
function Block({ block, emit }: { block: ViewBlock; emit: Emit }): React.ReactElement | null {
  switch (block.type) {
    ...
    case "table": return (
      <div className="overflow-x-auto"><table className="w-full text-[11.5px]"><thead><tr>{block.columns.map((c) => <th key={c.key} className={cn("pb-1.5 text-[10px] font-bold uppercase tracking-wide text-muted-foreground", c.align === "right" ? "text-right" : c.align === "center" ? "text-center" : "text-left")}>{c.label}</th>)}{block.rowAction && <th />}</tr></thead>
