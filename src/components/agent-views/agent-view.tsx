...
function Block({ block, emit }: { block: ViewBlock; emit: Emit }): React.ReactElement | null {
  switch (block.type) {
    ...
    case "table": return (
      ...
      <tbody>{block.rows.map((r, ri) => <tr key={ri}>{block.columns.map((c) => { const v = r[c.key]; return <td key={c.key} className={cn("border-t border-border py-1.5", c.align === "right" && "text-right", c.align === "center" && "text-center", c.kind === "score" && "font-mono font-bold text-emerald-500")}>{c.kind === "thumb" && typeof v === "string" ? <span className="inline-block h-6 w-6 overflow-hidden rounded"><Image src={v} alt="" width={24} height={24} className="h-full w-full object-cover" unoptimized /></span> : c.kind === "badge" ? <span className={cn("rounded px-1.5 py-0.5 text-[9.5px] font-bold", toneChipBg.brand)}>{String(v ?? "")}</span> : String(v ?? "")}</td>; })}{block.rowAction && <td className="border-t border-border py-1.5 text-right"><button onClick={() => block.rowAction && emit({ action: block.rowAction, value: r })} className="rounded-md border border-border px-2 py-0.5 text-[10px] font-bold hover:border-brand-500/60 hover:text-brand-500">{block.rowActionLabel || "Select"}</button></td>}</tr>)}</tbody></table></div>
    );
