...
const FOCUS_META: Record<string, { label: string; subtitle: string; icon: LucideIcon }> = {
  ...
  avatar: { label: "Avatar Studio", subtitle: "Talking avatars from your clone — powered by HeyGen", icon: UserSquare2 },
...
A "pitch" is a cold-outreach email (create_pitch); a "proposal" is a branded service deck (create_proposal) — pick the right one when asked.`;
    ...
    case "avatar":
      return `The user is on the **Avatar Studio** (HeyGen talking-avatar videos). Making an avatar video is a generative task — INTERVIEW the user first (what's the goal, tone, length, which avatar & voice), then write a punchy script for their goal and recommend a quality (Standard for social/outreach, Avatar IV for photoreal hero/ad content). Use create_avatar_video once you have a script + avatar + voice; it charges credits and renders into the studio. To make a reusable avatar or cloned voice from a photo/video, use clone_avatar (consent-gated).`;
    case "print":
