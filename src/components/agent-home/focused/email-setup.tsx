    ...
    <div className="mx-auto w-full max-w-xl">
      <div className="rounded-2xl border border-border bg-card p-5 sm:p-6">
        <div className="flex items-center gap-3">
          <span className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-gradient-to-br from-brand-500/20 to-violet-500/20 text-brand-500"><Mail className="h-5 w-5" /></span>
          <div>
            <h2 className="text-[17px] font-extrabold leading-tight">Set up email sending</h2>
            <p className="text-[12.5px] text-muted-foreground">Connect a provider and verify a sender — then you can send campaigns.</p>
          </div>
        </div>

        {/* step 1 — provider */}
        <div className="mt-4">
          <p className="mb-1.5 text-[11px] font-bold uppercase tracking-wide text-muted-foreground">1 · Sending provider</p>
          <div className="grid grid-cols-2 gap-1.5 sm:grid-cols-3">
            {PROVIDERS.map((p) => (
              <button key={p.v} onClick={() => pickProvider(p.v)} className={cn("rounded-[10px] border px-2.5 py-2 text-left transition", provider === p.v ? "border-brand-500 bg-brand-500/10" : "border-border hover:border-brand-500/40")}>
                <span className="block text-[12.5px] font-bold">{p.label}</span>
                <span className="block text-[10.5px] text-muted-foreground">{p.hint}</span>
              </button>
            ))}
          </div>
        ...
        {/* step 2 — sender */}
        <div className="mt-4">
          <p className="mb-1.5 text-[11px] font-bold uppercase tracking-wide text-muted-foreground">2 · Sender</p>
