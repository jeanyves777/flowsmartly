...
/**
 * Referrals — a deep new-design surface (the referral workspace canvas) in a
 * full-width master/detail layout: a sticky LEFT rail with the user's personal
 * referral link + code (one-tap copy + share), compact headline stats (referred,
 * converted, earned, pending), and a RIGHT pane with the people they brought in
 * plus the commission history — full width. Real data (GET /api/referrals).
 * Copy-to-share is the core action and it happens right here in the UI — no chat
 * prompt, no legacy links. [[new-design-no-legacy]] [[surface-buttons-are-ui-actions]]
 */
  ...
  return (
    <div className="min-h-0 flex-1 overflow-y-auto px-4 py-5 sm:px-6 lg:px-8">
      <div className="flex w-full flex-col gap-4 lg:flex-row lg:items-start">
        {/* LEFT: sticky summary + share rail */}
        <aside className="space-y-3 lg:sticky lg:top-0 lg:w-[280px] lg:shrink-0">
          {/* hero — link + code with copy + share */}
          <section className="rounded-2xl border border-brand-500/30 bg-gradient-to-br from-brand-500/10 via-violet-500/5 to-transparent p-4">
            <div className="flex items-start gap-2.5">
              <span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-gradient-to-br from-brand-500/25 to-violet-500/20 text-brand-500"><Gift className="h-5 w-5" /></span>
              <div className="min-w-0 flex-1">
                <p className="text-[11px] font-medium text-muted-foreground">Your referral program</p>
                <h2 className="text-[15px] font-extrabold leading-tight">Invite friends, earn together</h2>
              </div>
            </div>
            {stats.pendingCommissionsCents ? (
              <span className="mt-2.5 inline-flex items-center gap-1.5 rounded-full border border-border bg-background/60 px-3 py-1 text-[11.5px] font-semibold"><Clock className="h-3.5 w-3.5 text-amber-500" /> {money(stats.pendingCommissionsCents)} pending</span>
            ) : null}

            {error && !link && !code ? (
              <p className="mt-3 rounded-xl border border-dashed border-border bg-background/40 px-3 py-2.5 text-[12px] text-muted-foreground">{error}</p>
            ) : (
              <>
                {/* referral link */}
                <div className="mt-3">
                  <span className="mb-1 block text-[11px] font-medium text-muted-foreground">Your referral link</span>
                  <div className="flex min-w-0 items-center gap-2 rounded-[10px] border border-border bg-background/70 px-3 py-2.5">
                    <Link2 className="h-4 w-4 shrink-0 text-brand-500" />
                    <span className="truncate text-[12.5px] font-medium">{link || "—"}</span>
                  </div>
                  <button
                    onClick={() => copy(link, "link")}
                    disabled={!link}
                    className="mt-2 inline-flex w-full shrink-0 items-center justify-center gap-1.5 rounded-[10px] bg-gradient-to-r from-brand-500 to-violet-500 px-4 py-2.5 text-[13px] font-semibold text-white shadow-lg shadow-brand-500/30 disabled:opacity-60"
                  >
                ...
                <div className="mt-3">
                  ...
                  <p className="mt-2 text-[11.5px] leading-snug text-muted-foreground">When someone signs up with your link, you earn a recurring commission on their spend.</p>
                </div>

                {/* one-tap share */}
                <div className="mt-3 flex flex-wrap items-center gap-1.5">
                  <span className="text-[11px] font-medium text-muted-foreground">Share:</span>
                  <button onClick={() => share("x")} disabled={!link} className="inline-flex items-center gap-1.5 rounded-[10px] border border-border bg-background/70 px-2.5 py-1.5 text-[12px] font-semibold hover:border-brand-500/60 hover:text-foreground disabled:opacity-50"><Twitter className="h-3.5 w-3.5" /> X</button>
                  <button onClick={() => share("whatsapp")} disabled={!link} className="inline-flex items-center gap-1.5 rounded-[10px] border border-border bg-background/70 px-2.5 py-1.5 text-[12px] font-semibold hover:border-brand-500/60 hover:text-foreground disabled:opacity-50"><MessageCircle className="h-3.5 w-3.5" /> WhatsApp</button>
                  <button onClick={() => share("email")} disabled={!link} className="inline-flex items-center gap-1.5 rounded-[10px] border border-border bg-background/70 px-2.5 py-1.5 text-[12px] font-semibold hover:border-brand-500/60 hover:text-foreground disabled:opacity-50"><Mail className="h-3.5 w-3.5" /> Email</button>
                </div>
              </>
            )}
          </section>
        ...
        {/* RIGHT: referral history + commission history, full width */}
          ...
          <section className="rounded-2xl border border-border bg-card p-4 sm:p-5">
            ...
            {referrals.length ? (
              ...
              <div className="rounded-xl border border-dashed border-border px-4 py-8 text-center">
                ...
                <p className="mt-1 text-[12px] text-muted-foreground">Copy your link from the panel and share it — the people who sign up will show up here.</p>
                <button
                  onClick={() => copy(link, "link")}
                  disabled={!link}
...
function Kpi({ icon: Icon, label, value, tone }: { icon: ElementType; label: string; value: string; tone: string }) {
  return (
    <div className="rounded-xl border border-border bg-card p-2.5">
      ...
      <p className="mt-1 text-[16px] font-extrabold leading-none">{value}</p>
    </div>
  );
}
