...
import {
  ...
  ChevronDown, Play, Pause, FolderOpen, Wand2, Upload, Music, Captions as CaptionsIcon, Pencil, Maximize2, Volume2, VolumeX, RefreshCw, Link2, Trash2,
...
import { CastReviewSheet, type CastLibraryItem } from "@/components/agent-home/shared/cast-review";
...
import type { FilmComposer, FilmComposerLayer, FilmComposerLayerType, FilmProject, FilmScene, FilmOverlay, FilmAsset, SceneEngine, FilmType, FilmAspect, FilmCharacter, SceneCastLine } from "@/lib/video-director/types";
...
// same person appears across the film.
//
// The UI is the SHARED CastReviewSheet — every studio that anchors a recurring
// person renders the same surface. This only wires the Director's endpoints to it.
function CastPanel({ film, setFilm, onBuild, onClose }: {
  ...
  const { toast } = useToast();
  const fail = (title: string, msg?: string) => toast({ title, description: msg || "Please try again.", variant: "destructive" });
  const characters = film.characters || [];

  const patchCast = async (cid: string, body: Record<string, unknown>) => {
    try {
      const j = await fetch(`/api/ai/video-director/${film.id}/cast/${cid}`, {
        method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body),
      }).then((r) => r.json());
      if (j?.success && j.data?.film) { setFilm(j.data.film); return true; }
      fail("That change could not be saved", j?.error?.message);
    } catch { fail("That change could not be saved"); }
    return false;
  };

  // Wardrobe rides on the preview call so the outfit is applied atomically server-side.
  const genPreview = async (cid: string, opts?: { baseImageUrl?: string; wardrobe?: string }) => {
    try {
      const j = await fetch(`/api/ai/video-director/${film.id}/cast/${cid}/preview`, {
        method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(opts || {}),
      }).then((r) => r.json());
      if (j?.success && j.data?.film) setFilm(j.data.film);
      else fail("Character preview failed", j?.error?.message);
    } catch { fail("Character preview failed"); }
  };
  ...
  return (
    <CastReviewSheet
      ...
      onClose={onClose}
      usageNoun="scene"
      usageCount={(c) => film.scenes.filter((s) => s.cast?.some((l) => l.characterId === c.id || l.name.toLowerCase() === c.name.toLowerCase())).length}
      onGenerate={genPreview}
      onApprove={async (cid, approved) => { await patchCast(cid, { approved }); }}
      onSetRenderStyle={async (cid, renderStyle) => {
        const c = characters.find((x) => x.id === cid);
        if (!c || (c.renderStyle || "cinematic") === renderStyle) return;
        if (await patchCast(cid, { renderStyle })) {
          toast({ title: `${renderStyle === "3d" ? "3D" : "Cinematic"} style selected`, description: "Generate a new preview to lock this look." });
        }
      }}
      onRemove={async (cid) => {
        try {
          const j = await fetch(`/api/ai/video-director/${film.id}/cast/${cid}`, { method: "DELETE" }).then((r) => r.json());
          if (j?.success && j.data?.film) { setFilm(j.data.film); toast({ title: "Cast member removed" }); }
          else fail("Cast member could not be removed", j?.error?.message);
        } catch { fail("Cast member could not be removed"); }
      }}
      onDescribe={async (instruction, renderStyle) => {
        try {
          const j = await fetch(`/api/ai/video-director/${film.id}/cast/describe`, {
            method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ instruction, renderStyle }),
          }).then((r) => r.json());
          if (j?.success && j.data?.character) {
            toast({ title: "Character identity ready", description: "Review the details, then add or generate." });
            return { name: j.data.character.name || "", role: j.data.character.role || "", description: j.data.character.description || "" };
          }
          fail("AI Describe failed", j?.error?.message);
        } catch { fail("AI Describe failed"); }
        return null;
      }}
      onAdd={async (draft, generateNow) => {
        try {
          const j = await fetch(`/api/ai/video-director/${film.id}/cast/member`, {
            method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(draft),
          }).then((r) => r.json());
          if (!j?.success || !j.data?.film || !j.data?.character?.id) { fail("Cast member could not be added", j?.error?.message); return; }
          setFilm(j.data.film);
          toast({ title: "Cast member added", description: "This character is now available in every scene." });
          if (generateNow) await genPreview(j.data.character.id as string);
        } catch { fail("Cast member could not be added"); }
      }}
      library={{
        load: async () => {
          const j = await fetch(`/api/ai/video-director/cast-library?exclude=${film.id}`).then((r) => r.json());
          return j?.success && Array.isArray(j.data?.cast) ? (j.data.cast as CastLibraryItem[]) : [];
        },
        adopt: async (cid, sourceId) => {
          try {
      ...
      footer={{
        hint: "Approve all cast to storyboard the film with consistent characters.",
        skipLabel: "Skip cast", buildLabel: "Build film →", onBuild, requireAllApproved: true,
      }}
    />
  );
}
