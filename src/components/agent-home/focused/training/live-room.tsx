...
function Sheet({ title, sub, onClose, children }: { title: string; sub?: string; onClose: () => void; children: React.ReactNode }) {
  return (
    <>
      ...
      <div className="absolute inset-x-0 bottom-0 z-[31] flex max-h-[86%] flex-col rounded-t-3xl border-t border-border bg-card shadow-2xl">
        <span className="mx-auto mt-2 h-1 w-9 rounded-full bg-border" />
...
/* --------------------------------------------------------------- invite sheet */
function InviteSheet({ session, onClose }: { session: TrainingSessionDTO; onClose: () => void }) {
  ...
  const [open, setOpen] = useState<string | null>("link");
