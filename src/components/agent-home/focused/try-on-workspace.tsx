...
export function FocusedTryOn({ refreshKey }: { refreshKey?: number; onAsk?: (prompt: string) => void }) {
  ...
  // track dragging / resizing, exactly like the Director & Clone canvases.
    ...
    <div className="relative h-full w-full overflow-hidden" onPointerMove={onMove} onPointerUp={onUp}>
      ...
      <div ref={scrollRef} onPointerDown={pan} className="absolute inset-0 cursor-grab overflow-auto" style={{ backgroundImage: "radial-gradient(circle at 1px 1px, hsl(var(--border)) 1px, transparent 0)", backgroundSize: "24px 24px" }}>
        <div ref={boardRef} className="relative" style={{ width: 1900, height: 1200 }}>
          <svg className="pointer-events-none absolute inset-0 h-full w-full" style={{ overflow: "visible" }}><path d={wire} fill="none" stroke="#a855f7" strokeWidth={2} opacity={0.4} /></svg>
          {project && (
            <div data-wire="brief" data-nopan className="absolute w-[250px] overflow-hidden rounded-2xl border border-fuchsia-400/40 bg-card shadow-sm" style={{ left: 40, top: 60 }}>
          ...
          <PublishNode nodeId="__publish" channels={TRYON_CHANNELS} mediaKind="video" ready={stats.ready > 0}
            onOpen={() => {
      ...
      {publishTakeId && project && (
        <PublishSheet
          title="Publish try-on" subtitle={project.title} channels={TRYON_CHANNELS} mediaKind="video"
