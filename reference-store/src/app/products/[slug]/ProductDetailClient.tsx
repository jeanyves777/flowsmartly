...
// This is the CLIENT component — "use client" goes here.
// The server wrapper (page.tsx) exports generateStaticParams() and renders this.
      ...
      <main className="pt-24 pb-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        ...
        <div className="grid lg:grid-cols-2 gap-12">
                ...
                {product.images.map((img, i) => (
                  <button
                    ...
                    className={`w-20 h-20 rounded-xl overflow-hidden border-2 transition-colors ${
                      i === selectedImageIdx ? "border-primary-600" : "border-transparent hover:border-gray-300"
                    }`}
                  >
          ...
          <div>
            <div className="flex gap-2 mb-4">
              {product.badges.map(badge => (
                <span key={badge} className={`px-3 py-1 text-xs font-bold rounded-full ${
                  badge === "sale" ? "bg-red-100 text-red-600" :
                  badge === "new" ? "bg-primary-100 text-primary-600" :
                  badge === "bestseller" ? "bg-amber-100 text-amber-600" :
            ...
            <h1 className="text-3xl sm:text-4xl font-bold text-gray-900 dark:text-white mb-3">{product.name}</h1>
            ...
            <div className="flex items-center gap-4 mb-8">
              <div className="flex items-center border border-gray-200 dark:border-gray-700 rounded-full">
                <button onClick={() => setQuantity(Math.max(1, quantity - 1))} className="p-3 text-gray-500 hover:text-gray-700"><Minus size={16} /></button>
                <span className="w-10 text-center font-medium">{quantity}</span>
                <button onClick={() => setQuantity(quantity + 1)} className="p-3 text-gray-500 hover:text-gray-700"><Plus size={16} /></button>
              </div>
              <button
            ...
            <div className="grid grid-cols-3 gap-4 p-4 bg-gray-50 dark:bg-gray-800/50 rounded-2xl">
              <div className="text-center"><Package size={20} className="mx-auto text-primary-600 mb-1.5" /><p className="text-xs text-gray-500">Quality Assured</p></div>
              <div className="text-center"><Truck size={20} className="mx-auto text-primary-600 mb-1.5" /><p className="text-xs text-gray-500">Free Shipping $50+</p></div>
              <div className="text-center"><RotateCcw size={20} className="mx-auto text-primary-600 mb-1.5" /><p className="text-xs text-gray-500">30-Day Returns</p></div>
            </div>
          </div>
        </div>
